...
// Custom para multiroom
global $listing_meta_data;
$post_id = get_the_ID();
$listing_meta_data = get_post_meta( $post_id ); // o get_post_custom($post_id)

// Debe ejecutarse ANTES de llamar a homey_get_field_meta()
...
$prices = [];
$bedrooms = count($accomodation);
if (!empty($accomodation) && is_array($accomodation)) {
    foreach ($accomodation as $acc) {
        // Guardamos precios
        if (!empty($acc['night_price']) && is_numeric($acc['night_price'])) {
            $prices[] = floatval($acc['night_price']);
        }
    }
}
if (!empty($prices)) {
    ...
    <div class="media property-item">
        <div class="media-left">
            <div class="item-media item-media-thumb">
				...
				<?php if($multiroom_booking == 'per_day_multi') { ?>
					<div class="item-media-price">
                        <span class="item-price">
                            <?php echo homey_formatted_price($min_price) . ' a ' . homey_formatted_price($max_price);?><sub><?php echo esc_attr($price_separator); ?><?php echo homey_get_price_label();?></sub>
                        </span>
                    </div>
				<?php } ?>

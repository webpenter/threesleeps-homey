...
$multiroom_booking = get_post_meta($listing_data->ID, 'homey_multiroom_booking', true);
...
if ( ! empty( $listing_data ) && ( $listing_data->post_type == 'listing' ) ) {
    ...
    if ( ($listing_data->post_author == $current_user->ID) || homey_is_admin() ) {
        ...
        } elseif(isset($_GET['tab']) && $_GET['tab'] == 'pricing'  && $multiroom_booking != 'per_day_multi') {
...
<!--                        <button title="--><?php //echo esc_attr(ucwords($post_status)); ?><!--" disabled="disabled" class="btn btn-dark-grey disabled">--><?php //echo esc_attr($homey_local['update_btn']); ?><!--</button>-->
                    ...
                    if ($layout_order) {
                        foreach ($layout_order as $key=>$value) {
                            ...
                            switch($key) {
                                ...
                                case 'bedrooms':
                                    if ( $multiroom_booking === 'per_day_multi' ) {
                                        get_template_part('template-parts/dashboard/edit-listing/multiroom');
                                    } else {
            ...
            <input type="hidden" name="multiroom_booking" value="<?php echo esc_attr($multiroom_booking); ?>"/>

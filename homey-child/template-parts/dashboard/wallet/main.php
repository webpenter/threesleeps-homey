...
$stripe->display_stripe_connection_button();

$stripe->display_stripe_account_status();

if ( isset( $_POST['stripe_disconnect'] ) && wp_verify_nonce( $_POST['stripe_disconnect_nonce'], 'stripe_disconnect' ) ) {
    $result = $stripe->disconnect_stripe_connect_account();
    if ( is_wp_error( $result ) ) {
        echo esc_html( $result->get_error_message() );
    } else {
        echo esc_html__( 'Your Stripe account has been disconnected.', 'homey-core' );
    }
}

$stripe_account_id = get_user_meta($userID, 'stripe_connect_account_id', true);
$balance_data = false;

if (!empty($stripe_account_id)) {
    $balance_data = $stripe->get_stripe_balance($stripe_account_id);

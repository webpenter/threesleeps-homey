...
if($_GET['mode']=='per_day_multi'){
    $_GET['mode'] = 'per_day';
    $_GET['mode_multi'] = 'per_day_multi';
}
...
<form autocomplete="off" id="submit_listing_form" name="new_post" method="post" action="#" enctype="multipart/form-data" class="add-frontend-property">
    ...
    if ($layout_order) { 
        foreach ($layout_order as $key=>$value) {
            ...
            switch($key) { 
                ...
                case 'pricing':
                    if (isset($_GET['mode_multi']) && $_GET['mode_multi'] === 'per_day_multi') {
                        break; // just skip it
                    }
                ...
                case 'bedrooms':
                    if ( isset($_GET['mode_multi']) && $_GET['mode_multi'] === 'per_day_multi' ) {
                        get_template_part('template-parts/dashboard/submit-listing/multiroom');
                    } else {
    ...
    <input type="hidden" name="multiroom_booking" value="<?php echo esc_attr($_GET['mode_multi']); ?>"/>

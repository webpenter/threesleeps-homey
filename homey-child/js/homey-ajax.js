jQuery(document).ready(function ($) {
    ...
    if ( typeof HOMEY_ajax_vars !== "undefined" ) {
        ...
        var homey_calculate_booking_cost = function(check_in_date, check_out_date, guests, listing_id, security, extra_options, adult_guest, child_guest, selectedRooms) {
            var $this = $(this);
            var notify = $('.homey_notification');
            notify.find('.notify').remove();

            var multi_listing = $('#multi_listing').val();
            ...
            $.ajax({
                ...
                success: function(data) {
                    if( data.success ) {
                        ...
                        $(".selectpicker.listing_multirooms").html(data.options);

                        // 2. Replace lis inside the bootstrap dropdown UL
                        $(".selectpicker.listing_multirooms")
                            .parent() // div.btn-group
                            .find("ul.dropdown-menu.inner")
                            .html(data.lis);

                        // 3. Refresh bootstrap-select to apply
                        $(".selectpicker.listing_multirooms").selectpicker("refresh");
                    } else {
        ...
        if(homey_booking_type != 'per_hour') {
            ...
            * */
            ...
            function resetSelectIfSelected(selectId) {
                ...
                if (select && select.value) {
                    ...
                    // For multiple select, clear all selected options
                    Array.from(select.options).forEach(option => option.selected = false);

                    // If using bootstrap-select (like your code)
            ...
            var homey_calculate_price_checkout = function() {
                ...
                resetSelectIfSelected("listing_multirooms");
            ...
            $('.listing_multirooms').on('change', function () {
                ...
                let selectedRooms = $(this).val();
                
                if(selectedRooms === '')
                    return;

                homey_calculate_booking_cost(check_in_date, check_out_date, guests, listing_id, security, extra_options, adult_guest, child_guest, selectedRooms);
        ...
        $('#request_for_reservation, #request_for_reservation_mobile').on('click', function(e){
            ...
            var multi_listing = $('#multi_listing').val();
            if(multi_listing == ''){
                multi_listing = $('#overlay-booking-module').find('#multi_listing').val();
            }

            let selectedRooms = $('.selectpicker.listing_multirooms').val();

            if(selectedRooms == ''){
                selectedRooms = $('#overlay-booking-module').find('.selectpicker.listing_multirooms').val();
            }
        ...
        * -------------------------------------------------------------------------*/
        ...
        $('#make_woocommerce_instant_booking_payment').on('click', function(e) {
            ...
            let selectedRoomInput = document.getElementById("selectedRoom").value;
            let selectedRooms = JSON.parse(selectedRoomInput);

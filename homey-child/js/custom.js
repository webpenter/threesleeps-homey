    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        document.querySelectorAll('.amenity-toggle').forEach(btn => {
            btn.addEventListener('click', function() {
                const amenitiesList = this.closest('.amenities-list');
                const extraAmenities = amenitiesList.querySelectorAll('.amenity-extra'); // permanent group
                const toggleText = this.querySelector('.toggle-text');
                const isExpanded = this.classList.contains('expanded');

                if (isExpanded) {
                    // Collapse → hide extra amenities
                    extraAmenities.forEach(amenity => {
                        amenity.classList.add('amenity-hidden');
                    });
                    toggleText.textContent = `Ver más (+${extraAmenities.length})`;
                    this.classList.remove('expanded');
                } else {
                    // Expand → show extra amenities
                    extraAmenities.forEach(amenity => {
                        amenity.classList.remove('amenity-hidden');
                    });

jQuery(document).ready( function($) {
    ...
    if ( typeof Homey_Listing !== "undefined" ) {
        ...
        var reservation_del_verify_nonce = Homey_Listing.reservation_del_verify_nonce;
        ...
        var listing_size_label = Homey_Listing.listing_size_label;
        var listing_size_plac = Homey_Listing.listing_size_plac;
        var listing_size_unit_label = Homey_Listing.listing_size_unit_label;
        var listing_size_unit_plac = Homey_Listing.listing_size_unit_plac;
        var price_label = Homey_Listing.price_label;
        var price_plac = Homey_Listing.price_plac;
        var cleaning_fee_label = Homey_Listing.cleaning_fee_label;
        var cleaning_fee_plac = Homey_Listing.cleaning_fee_plac;
        var fees_label = Homey_Listing.fees_label;
        var per_stay_text = Homey_Listing.per_stay_text;
        var city_fee_label = Homey_Listing.city_fee_label;
        var city_fee_plac = Homey_Listing.city_fee_plac;
        var ad_drag_drop_img = Homey_Listing.ad_drag_drop_img;
        var ad_image_size_text = Homey_Listing.ad_image_size_text;
        var upload_btn_text = Homey_Listing.upload_btn_text; 
        var acc_btn_remove_room = Homey_Listing.acc_btn_remove_room;
                    ...
                    if(result==true) {
                        ...
                        $.ajax({
                            ...
                            data: {
                                ...
                                'security': reservation_del_verify_nonce
                            },
                            beforeSend: function( ) {
                                $this.find('i').removeClass('fa-trash');
                                $this.find('i').addClass('fa-spin fa-spinner');
        ...
        let roomIndex = $('#homey_accomodation_count').val() || 0;

        // ====== Uploader Factory ======
        function createUploader(buttonId, containerId, errorsId, progressId, id) {
            $(`#${containerId}`).sortable({ placeholder: "sortable-placeholder" });
            ...
            uploader.bind('FilesAdded', function(up, files) {
                if (up.files.length > 50) {
                    up.splice(50);
                    alert('No more than 50 files allowed');
                    return;
                }
                let thumbs = files.map(f =>
                    `<div id="thumb-holder-${f.id}" class="col-sm-2 col-xs-4 listing-thumb"></div>`
                ).join('');
                $(`#${containerId}`).append(thumbs);
                up.refresh();
                uploader.start();
            });
            ...
            uploader.bind('UploadProgress', function(up) {
                $(`#${progressId}`).html(
                    `<span class="alert alert-info">${up.total.uploaded} Uploaded Of ${up.files.length}</span>`
                );
            });

            uploader.bind('Error', function(up, err) {
                $(`#${errorsId}`).append(`<br/>Error #${err.code}: ${err.message}`);
            });

            uploader.bind('FileUploaded', function(up, file, ajax_response) {
                const response = $.parseJSON(ajax_response.response);
                let html = '';
                ...
                if (response.success) {
                    html = `
                        ...
                        <div class="upload-gallery-thumb-buttons">
                            <button class="icon-delete" data-attachment-id="${response.attachment_id}">
                                <i class="homey-icon homey-icon-bin-1-interface-essential"></i>
                            </button>
                            <input type="hidden" name="homey_accomodation[${id}][select_gallery_images_room][]" value="${response.attachment_id}">
                        </div>
                        <span class="icon icon-loader" style="display:none;">
                            <i class="homey-icon homey-icon-loading-half fa-spin"></i>
                        </span>
                    `;
                    $(`#thumb-holder-${file.id}`).html(html);
                    lisitng_thumbnail_event();
                } else {
                    $(`#${errorsId}`).append(`<span class="error-message">${response.reason}</span>`);
                }

                if (uploader.files.length === (uploader.total.uploaded + uploader.total.failed)) {
                    $(`#${progressId}`).html('<span class="alert alert-success">Process completed.</span>');
                }
            });
        ...
        function renderBedroom(id) {
            const room_id = "room_" + Date.now() + "_" + Math.floor(Math.random() * 10000);
            return `
            <div class="more_rooms_wrap">
                <input type="hidden" name="homey_accomodation[${id}][room_id]" value="${room_id}">

                <div class="row">
                    <div class="col-sm-12">
                        <div class="form-group">
                            <label>${acc_bedroom_name}</label>
                            <input type="text" name="homey_accomodation[${id}][acc_bedroom_name]" class="form-control" placeholder="${acc_bedroom_name_plac}">
                        </div>
                    </div>
                </div>
                ...
                <div class="row">
                    <div class="col-sm-6">
                        <div class="form-group">
                            <label>${acc_no_of_beds}</label>
                            <input type="text" name="homey_accomodation[${id}][acc_no_of_beds]" class="form-control" placeholder="${acc_no_of_beds_plac}">
                        </div>
                    </div>
                    <div class="col-sm-6">
                        <div class="form-group">
                            <label>${acc_guests}</label>
                            <input type="text" name="homey_accomodation[${id}][acc_guests]" class="form-control" placeholder="${acc_guests_plac}">
                        </div>
                    </div>
                </div>

                <!-- Optional listing size -->
                <div class="row">
                    <div class="col-sm-6">
                        <div class="form-group">
                            <label>${listing_size_label}</label>
                            <input type="text" name="homey_accomodation[${id}][listing_size]" class="form-control" placeholder="${listing_size_plac}">
                        </div>
                    </div>
                    <div class="col-sm-6">
                        <div class="form-group">
                            <label>${listing_size_unit_label}</label>
                            <input type="text" name="homey_accomodation[${id}][listing_size_unit]" class="form-control" placeholder="${listing_size_unit_plac}">
                        </div>
                    </div>
                </div>
                ...
                <div class="row">
                    <div class="col-sm-12"><h3>Pricing</h3></div>
                    <div class="col-sm-12">
                        <div class="form-group">
                            <label>${price_label}</label>
                            <input type="text" name="homey_accomodation[${id}][night_price]" class="form-control" placeholder="${price_plac}">
                        </div>
                    </div>
                </div>
                ...
                <!-- Cleaning Fee -->
                <div class="row">
                    <div class="col-sm-12"><label>${cleaning_fee_label}</label></div>
                    <div class="col-sm-6">
                        <div class="form-group">
                            <input type="text" name="homey_accomodation[${id}][cleaning_fee]" class="form-control" placeholder="${cleaning_fee_plac}">
                        </div>
                    </div>
                    <div class="col-sm-3 col-xs-6">
                        <div class="form-group">
                            <label class="control control--radio radio-tab">
                                <input type="radio" name="homey_accomodation[${id}][cleaning_fee_type]" value="daily">
                                <span class="control-text">${fees_label}</span>
                    ...
                    <div class="col-sm-3 col-xs-6">
                        <div class="form-group">
                            <label class="control control--radio radio-tab">
                                <input type="radio" name="homey_accomodation[${id}][cleaning_fee_type]" value="per_stay" checked="checked">
                                <span class="control-text">${per_stay_text}</span>
                                <span class="control__indicator"></span>
                                <span class="radio-tab-inner"></span>
                            </label>
                        </div>
                ...
                <!-- City Fee -->
                <div class="row">
                    <div class="col-sm-12"><label>${city_fee_label}</label></div>
                    <div class="col-sm-6">
                        <div class="form-group">
                            <input type="text" name="homey_accomodation[${id}][city_fee]" class="form-control" placeholder="${city_fee_plac}">
                        </div>
                    </div>
                    <div class="col-sm-3 col-xs-6">
                        <div class="form-group">
                            <label class="control control--radio radio-tab">
                                <input type="radio" name="homey_accomodation[${id}][city_fee_type]" value="daily">
                                <span class="control-text">${fees_label}</span>
                                <span class="control__indicator"></span>
                                <span class="radio-tab-inner"></span>
                            </label>
                    ...
                    <div class="col-sm-3 col-xs-6">
                        <div class="form-group">
                            <label class="control control--radio radio-tab">
                                <input type="radio" name="homey_accomodation[${id}][city_fee_type]" value="per_stay" checked="checked">
                                <span class="control-text">${per_stay_text}</span>
                                <span class="control__indicator"></span>
                                <span class="radio-tab-inner"></span>
                            </label>
					...
					<div class="col-sm-12 col-xs-12">
                        <div class="form-group">
                            <label for="listing_payout"> Payment while booking </label>
                            <input type="text" class="form-control" name="homey_accomodation[${id}][listing_payout]" placeholder="Check how much initial payment you want the client to deposit while booking">
                        </div>
                    </div>
					<div class="col-sm-12 col-xs-12">
                        <div class="form-group">
                            <label for="listing_ical"> iCal Link </label>
                            <input type="text" class="form-control"  name="homey_accomodation[${id}][listing_ical]" placeholder="feed_name|iCal url , feed_name|second iCal url">
                        </div>
                    </div> 
                </div>
                <hr class="row-separator">
                ...
                <div class="listing-form-row">
                    <div class="house-features-list aminities0">
                        <label class="label-title">${Homey_Listing.amenities_label}</label>
                        ${Homey_Listing.amenities.map((amenity) => `
                            <label class="control control--checkbox">
                                <input 
                                    type="checkbox" 
                                    name="homey_accomodation[${id}][listing_amenities][]" 
                                    id="amenity-${amenity.slug}" 
                                    value="${amenity.slug}">
                                <span class="contro-text">${amenity.name}</span>
                                <span class="control__indicator"></span>
                            </label>
                        `).join('')}
                    </div>
                ...
                <div class="listing-form-row">
                    <div class="house-features-list facilities0">
                        <label class="label-title">${Homey_Listing.facilities_label}</label>
                        ${Homey_Listing.facilities.map((facility) => `
                        <label class="control control--checkbox">
                            <input 
                                type="checkbox" 
                                name="homey_accomodation[${id}][listing_facilities][]" 
                                id="facility-${facility.slug}" 
                                value="${facility.slug}">
                            <span class="contro-text">${facility.name}</span>
                            <span class="control__indicator"></span>
                        </label>
                    `).join('')}
                ...
                <!-- Upload (with icon + heading) -->
                <div id="homey_gallery_dragDrop_room_${id}" class="media-drag-drop">
                    <div class="upload-icon">
                        <i class="homey-icon homey-icon-picture-landscape-images-photography" aria-hidden="true"></i>
                    </div>
                    <h4>
                        ${ad_drag_drop_img}<br>
                        <span>${ad_image_size_text}</span>
                    </h4>
                    <button id="select_gallery_images_room_${id}" type="button" class="btn btn-secondary">
                        <i class="homey-icon homey-icon-social-media-yelp"></i> ${upload_btn_text}
                    </button>
                </div>

                <div id="homey_gallery_container_room_${id}" class="row"></div>
                <div id="homey_errors_${id}"></div>
                <div id="upload-progress-images_room_${id}" class="progree_system"></div>


                <!-- Remove Button -->
                <button type="button" class="btn btn-danger remove-beds" data-remove="${id}">${acc_btn_remove_room}</button>
                <hr>
            </div>
            `;
        }

        const roomInputs = document.querySelectorAll('[id^="select_gallery_images_room_"]');

        roomInputs.forEach((input) => {
            // Extract the index from the id
            const match = input.id.match(/select_gallery_images_room_(\d+)/);
            if (match) {
                const roomIndex = match[1]; // captured number
                createUploader(
                    `select_gallery_images_room_${roomIndex}`,
                    `homey_gallery_container_room_${roomIndex}`,
                    `homey_errors_${roomIndex}`,
                    `upload-progress-images_room_${roomIndex}`,
                    roomIndex
                );
            }
        });
        ...
        // ====== Add Bedroom ======
        $('#add_multi_bedrooms').on('click', function(e) {
            e.preventDefault();
            roomIndex++;
            $('#more_bedrooms_main').append(renderBedroom(roomIndex));

            // Init uploader for this room
            createUploader(
                `select_gallery_images_room_${roomIndex}`,
                `homey_gallery_container_room_${roomIndex}`,
                `homey_errors_${roomIndex}`,
        ...
        // ====== Remove Bedroom ======
        $(document).on('click', '.remove-beds', function() {
            $(this).closest('.more_rooms_wrap').remove();
        });

        // ====== Sadiq Multi-Bedroom End ====== //

 ...
 */
if ( ! function_exists( 'enqueue_notifyjs_scripts' ) ) {
    function enqueue_notifyjs_scripts() {
        ...
        wp_enqueue_script(
            ...
            array('jquery'),
            '0.4.2',
            true // This sets script to be loaded in footer
        );
    }
}
 ...
 */
if ( ! function_exists( 'homey_notify_message' ) ) {
 ...
 */
if ( ! function_exists( 'homey_custom_notify_css' ) ) {
 ...
 */
if ( ! function_exists( 'homey_notify_with_button_custom' ) ) {
    function homey_notify_with_button_custom($title, $button_text, $button_url) {
        ...
        <script>
        jQuery(function($) {
            $.notify.addStyle('foo', { html: `<?= preg_replace('/\s+/', ' ', $markup) ?>` });
            ...
            $.notify(
                { title: "<?= esc_js($title) ?>", button: "<?= esc_js($button_text) ?>" },
                <?= wp_json_encode($config, JSON_HEX_TAG) ?>
            );
        });
        </script>
 ...
 */
if ( ! function_exists( 'handle_create_stripe_account' ) ) {
 ...
 */
if ( ! function_exists( 'homey_admin_remove_stripe_account_page' ) ) {
 ...
 */
if ( ! function_exists( 'homey_remove_payout_accounts_callback' ) ) {
 ...
 */
if ( ! function_exists( 'homey_admin_disconnect_user_payout_account' ) ) {
...
if ( ! function_exists( 'child_add_stripe_client_id_field' ) ) {

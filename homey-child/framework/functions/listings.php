...
//            if (($check_is_approved == 0 || $edit_listings_admin_approved != 0) && !homey_is_admin()) {
        ...
        if ($listing_id > 0) {
			...
			// Bedrooms
            if (isset($_POST['listing_payout_booking'])) {
                update_post_meta($listing_id, $prefix . 'listing_payout_booking', sanitize_text_field($_POST['listing_payout_booking']));
            }
            ...
            if (isset($_POST['multiroom_booking'])) {
                update_post_meta($listing_id, $prefix . 'multiroom_booking', sanitize_text_field($_POST['multiroom_booking']));
            }
			...
			// Cancellation max days
            if (isset($_POST['cancel_max_days'])) {
                update_post_meta($listing_id, $prefix . 'cancel_max_days', sanitize_textarea_field($_POST['cancel_max_days']));
            }
...
if (!function_exists('homey_create_invoice_print')) {
    function homey_create_invoice_print()
    {
        ...
        $all_meta = get_user_meta( $invoice_data['invoice_resv_owner'] ); // returns array: [ 'meta_key' => [ value1, value2 ], ... ]
        $host_name = $all_meta['first_name'][0] . ' ' . $all_meta['last_name'][0];
        $host_address = $all_meta['cif'][0] . '<br> '. $all_meta['homey_street_address'][0] . '<br> ' . $all_meta['homey_city'][0] . ', ' . $all_meta['homey_state'][0] . '<br> ' . $all_meta['homey_zipcode'][0] . ', ' . $all_meta['homey_country'][0];
        ...
        <div class="invoice-detail block">
            <div class="invoice-header clearfix">
                <div class="block-left">
                    ...
                    <ul class="list-unstyled">
                        <?php if( !empty($host_name) ) { ?>
                            <li><strong><?php echo ($host_name); ?></strong></li>
                        <?php } ?>
                        <li><?php echo $host_address; ?></li>
                    </ul>
                </div>
            ...
            <?php if (!empty($invoice_additional_info) && 1==2) { ?>

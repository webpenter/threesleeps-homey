...
if(!function_exists('homeyGenerateMonth')) {
	function homeyGenerateMonth( $numberOfMonths, $unixMonth, $currentMonth, $currentYear, $listing_id = -1 ) {
        ...
        $pending_dates_rooms  = get_post_meta($listing_id, 'reservation_pending_dates_rooms',true  );
        $booked_dates_rooms  = get_post_meta($listing_id, 'reservation_dates_rooms',true  ); 

		$normalized = [];
        if(!empty($booked_dates_rooms)) {
            foreach ($booked_dates_rooms as $timestamp => $rooms) {
		...
		for ( $day = 1; $day <= $daysInMonth; ++$day ) {
            ...
            if (!empty($roomId) && is_array($roomId)) {
                // Get rooms for this day
                $pendRoomsForDay = $pending_dates_rooms[$timestamp] ?? [];
                $bookRoomsForDay = $booked_dates_rooms[$timestamp] ?? [];

                // Check if ALL rooms are in pending
                if (!empty($pendRoomsForDay) && count(array_intersect($roomId, $pendRoomsForDay)) === count($roomId)) {
                    $allRoomsPending = true;
                }
                // Check if ALL rooms are in booked
                elseif (!empty($bookRoomsForDay) && count(array_intersect($roomId, $bookRoomsForDay)) === count($roomId)) {
                    $allRoomsBooked = true;
                }
                // If some pending and some booked → consider as pending
                elseif (
                    !empty(array_intersect($roomId, $pendRoomsForDay)) &&
                    !empty(array_intersect($roomId, $bookRoomsForDay))
                ) {
                    $allRoomsPending = true;
                }
            }
            ...
            } elseif(  $allRoomsPending ||  array_key_exists($timestamp, $pending_dates) && !array_key_exists($timestamp, $bookedDays) ) {
            ...
            }  elseif( $allRoomsBooked ||  array_key_exists($timestamp, $bookedDays) ) {

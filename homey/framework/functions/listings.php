...
//            if (($check_is_approved == 0 || $edit_listings_admin_approved != 0) && !homey_is_admin()) {
        ...
        if ($listing_id > 0) {
            ...
            if (isset($_POST['additional_rules'])) {
                update_post_meta($listing_id, $prefix . 'additional_rules', wp_kses_post($_POST['additional_rules']));
            }
...
if (!function_exists('save_listing_as_draft')) {
    function save_listing_as_draft()
    {
        ...
        if (isset($_POST['draft_listing_id']) && !empty($_POST['draft_listing_id'])) {
            if(homey_is_listing_owner($_POST['draft_listing_id'], $userID)){
                $new_listing['ID'] = $_POST['draft_listing_id'];
                $listing_id = wp_update_post($new_listing);
            }else{
                echo json_encode(array('success' => false, 'listing_id' => $listing_id, 'msg' => esc_html__('This is not owner listing? Something bad is happend, please try again.', 'homey')));
                wp_die();
            }
        } else {
            $listing_id = wp_insert_post($new_listing);
        ...
        if ($listing_id > 0) {
            ...
            if (isset($_POST['room_type']) && ($_POST['room_type'] != '-1')) {
                ...
                // Unset the listing_type for the listing
                wp_remove_object_terms($listing_id, 'room_type', 'room_type');
            }
            ...
            if (isset($_POST['listing_type']) && ($_POST['listing_type'] != '-1')) {
                ...
                // Unset the listing_type for the listing
                wp_remove_object_terms($listing_id, 'listing_type', 'listing_type');
            }
...
if (!function_exists('homey_delete_listing')) {
    ...
    function homey_delete_listing()
    {
        ...
        $listing_id = $_REQUEST['listing_id'];
        $post_author = get_post_field('post_author', $listing_id);
        $is_on_listing_delete = homey_option('on_listing_delete', 1);

        global $current_user;
        wp_get_current_user();

        if (($post_author == $userID) || homey_is_admin()) {
            if ($is_on_listing_delete == 0) {
                wp_trash_post($listing_id);
                $ajax_response = array('success' => true, 'reason' => esc_html__('listing trashed, you can restore it anytime.', 'homey'));
                echo json_encode($ajax_response);
            } else {
...
if (!function_exists('homey_listing_sort')) {
    function homey_listing_sort($query_args)
    {
        ...
        if ($sort_by == 'a_price') {
            ...
            if (isset($_GET['max-price']) && !empty($_GET['max-price']) && $_GET['max-price'] != 'any'){
                            ...
                            <div class="block-right">
                                <span class="item-price">
                                    <?php if ($booking_type == 'per_hour') { ?>
                                        ...
                                        /<?php echo homey_get_price_label(); //homey_option('glc_hour_label'); ?></sub>
                                    <?php } else { ?>
                                        <?php echo homey_formatted_price($night_price, false, true); ?><sub>
                                        /<?php echo homey_get_price_label(); // homey_option('glc_day_night_label'); ?></sub>
                                    <?php } ?>
                                </span>
                            </div><!-- block-right -->
                        </div><!-- block-head -->
...
if (!function_exists('homey_put_hold_listing')) {
    ...
    function homey_put_hold_listing()
    {
        ...
        if(homey_is_listing_owner($listing_id, $userID) < 1){
            echo json_encode(array('success' => false, 'listing_id' => $listing_id, 'reason' => esc_html__('Something bad is happend, please try again.', 'homey')));
            wp_die();
        }
...
if (!function_exists('add_expired_status_to_admin')) {
    ...
    add_action('pre_get_posts', 'add_expired_status_to_admin');
    function add_expired_status_to_admin($query) {
        // Check if it's the admin dashboard and the query is for your CPT

        if (is_admin() && $query->is_main_query() && $query->get('post_type') === 'listing') {
            // Include 'expired' status in the query
            $post_status = $query->get('post_status');

            if (empty($post_status) || $post_status === 'all') {
                $query->set('post_status', ['publish', 'draft', 'expired']);
            }
        }
    }
    // Show the states
    add_filter('display_post_states', 'add_expired_post_state');
    function add_expired_post_state($post_states) {
        global $post;
        if(!is_null($post)){
            if ($post->post_status === 'expired') {
                $post_states['expired'] = __('Expired');
            }
        }
    ...
    //filter
    add_filter('views_edit-listing', 'add_expired_to_status_filter');
    function add_expired_to_status_filter($views) {
        global $wp_query;

        // Count posts with the 'expired' status
        $expired_count = isset($wp_query->query_vars['post_status']) ? count(get_posts([
            'post_type'   => 'listing',
            'post_status' => 'expired',
        ])) : 0;

        // Add 'expired' link to the views
        $views['expired'] = sprintf(
            '<a href="%s" class="%s">Expired (%d)</a>',
            admin_url('edit.php?post_type=listing&post_status=expired'),
            (isset($_GET['post_status']) && $_GET['post_status'] === 'expired') ? 'current' : '',
            $expired_count
        );
...
//styles
    add_action('admin_head', 'style_expired_listing');
    function style_expired_listing() {
        echo '<style>
        .post-state.expired { color: #ff0000; font-weight: bold; }
    </style>';
    }
